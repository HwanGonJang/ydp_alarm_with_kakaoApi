...
def keyboard(request):
    ...
    return JsonResponse({
        'type':'buttons',
        'buttons':['급식','오늘의 명언','시간표','영등포고등학교 홈페이지','변경사항','개발정보']
    })
...
def message(request):
    ...
    if datacontent == '급식':
        ...
        return JsonResponse({
                ...
                'keyboard': {
                    'type':'buttons',
                    'buttons':['우리 반 추가하기','1학년7반','2학년4반','2학년6반']
                }
            })
    ...
    elif datacontent == '우리 반 추가하기':

        return JsonResponse({
                'message': {
                    'text': '모든 반이 없는 이유\n>>사용자가 없는 반은 넣어둘 필요가 없다고 판단해서 삭제되었고, 언제든지 1대1상담으로 시간표 이미지만 첨부해 주면 추가할수 있습니다. '
                },
                'keyboard': {
                    'type':'buttons',
                    'buttons':['처음으로']
